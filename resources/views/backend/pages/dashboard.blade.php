@extends('backend.layout.master')

@section('content')
{{-- breadcrumb part start --}}
<div class="bread-crumb-part">
  <div class="row">
    <div class="col-lg-6 small-device-dash">
      <div class="left-title">
        <h3>Dashboard</h3>
      </div>
    </div>
    <div class="col-lg-6 small-device-breadcrumb">
      <div class="right-info text-end">
        <ul>
          <li><a href="index.html">Dashboard</a></li>
          <li>Widget</li>
          <li>Otros buscadores</li>
        </ul>
      </div>
    </div>
  </div>
</div>
{{-- breadcrumb part end --}}


<div class="content-part">
        <div class="top-short-summery">
          <div class="row">
            <div class="col-lg-6 col-xl-3 col-xxl-3 col-md-6">
              <div class="summery-item">
                <h4>Total Accounts</h4>
                <h3 class="counter">586,356</h3>
                <ul>
                  <li>
                    <span><i class="bi bi-arrow-up"></i> 30.4%</span> increase this month
                  </li>
                </ul>
                <div class="circle">
                  <i class="far fa-id-card" aria-hidden="true"></i>
                </div>
              </div>
            </div>
            <div class="col-lg-6 col-xl-3 col-xxl-3 col-md-6">
              <div class="summery-item item-2">
                <h4>Products Sold</h4>
                <h3 class="counter">158,356</h3>
                <ul>
                  <li>
                    <span><i class="bi bi-arrow-up"></i> 15.4%</span> increase this month
                  </li>
                </ul>
                <div class="circle">
                  <i class="far fa-lightbulb" aria-hidden="true"></i>
                </div>
              </div>
            </div>
            <div class="col-lg-6 col-xl-3 col-xxl-3 col-md-6">
              <div class="summery-item item-3 mb-md-0">
                <h4>Net Profit</h4>
                <h3 class="counter">586,356</h3>
                <ul>
                  <li>
                    <span><i class="bi bi-arrow-up"></i> 15.4%</span> increase this month
                  </li>
                </ul>
                <div class="circle">
                  <i class="fas fa-recycle" aria-hidden="true"></i>
                </div>
              </div>
            </div>
            <div class="col-lg-6 col-xl-3 col-xxl-3 col-md-6">
              <div class="summery-item item-4">
                <h4>Customer Satisfaction</h4>
                <h3><span class="counter">99</span> <span>%</span></h3>
                <ul>
                  <li>
                    <span><i class="bi bi-arrow-up"></i> 15.4%</span> increase this month
                  </li>
                </ul>
                <div class="circle">
                  <i class="far fa-snowflake" aria-hidden="true"></i>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div class="details-info">
          <div class="row">
            <div class="col-lg-12 col-xl-3 col-xxl-3">
              <div class="info-notification">
                <div class="title">
                  <h4>Notifications</h4>
                </div>
                <div class="info-item-main">
                  <div class="row">
                    <div class="col-md-6 col-lg-6 col-xl-12 col-xxl-12">
                      <div class="info-notify-item d-flex">
                        <div class="icon">
                          <i class="fab fa-think-peaks" aria-hidden="true"></i>
                        </div>
                        <div class="text">
                          <h4>
                            <a href="#" class="d-block">
                              Group lunch celebration
                            </a>
                            <span>Due in 2 Days</span>
                          </h4>
                        </div>
                        <div class="percent">
                          <h5>+28%</h5>
                        </div>
                      </div>
                    </div>
                    <div class="col-md-6 col-lg-6 col-xl-12 col-xxl-12">
                      <div class="info-notify-item info-item2 d-flex">
                        <div class="icon">
                          <i class="fab fa-think-peaks" aria-hidden="true"></i>
                        </div>
                        <div class="text">
                          <h4>
                            <a href="#" class="d-block">
                              Group lunch celebration
                            </a>
                            <span>Due in 2 Days</span>
                          </h4>
                        </div>
                        <div class="percent">
                          <h5>+28%</h5>
                        </div>
                      </div>
                    </div>
                    <div class="col-md-6 col-lg-6 col-xl-12 col-xxl-12">
                      <div class="info-notify-item info-item3 d-flex">
                        <div class="icon">
                          <i class="fab fa-think-peaks" aria-hidden="true"></i>
                        </div>
                        <div class="text">
                          <h4>
                            <a href="#" class="d-block">
                              Group lunch celebration
                            </a>
                            <span>Due in 2 Days</span>
                          </h4>
                        </div>
                        <div class="percent">
                          <h5>+28%</h5>
                        </div>
                      </div>
                    </div>
                    <div class="col-md-6 col-lg-6 col-xl-12 col-xxl-12">
                      <div class="info-notify-item mb-0 info-item4 d-flex">
                        <div class="icon">
                          <i class="fab fa-think-peaks" aria-hidden="true"></i>
                        </div>
                        <div class="text">
                          <h4>
                            <a href="#" class="d-block">
                              Group lunch celebration
                            </a>
                            <span>Due in 2 Days</span>
                          </h4>
                        </div>
                        <div class="percent">
                          <h5>+28%</h5>
                        </div>
                      </div>
                    </div>
                  </div>




                </div>
              </div>
            </div>
            <div class="col-lg-12 col-xl-9 col-xxl-9">
              <div class="projects">
                <div class="title">
                  <h4>Latest Projects</h4>
                </div>
                <div class="project-table table-responsive">
                  <table class="table">
                    <thead>
                      <tr>
                        <th>#</th>
                        <th>Project Name</th>
                        <th>Start Date</th>
                        <th>End Date</th>
                        <th>Status</th>
                        <th>Working Hour</th>
                        <th>Assign To</th>
                        <th>Action</th>
                      </tr>
                    </thead>
                    <tbody style="border-top: 0;">
                      <tr>
                        <td>1</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released">Released</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>2</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released pending">Pending</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>3</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released in-progress">Work In Progress</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>4</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released comming">Coming soon</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>5</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released finish">Finished</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>6</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released">Released</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>7</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released pending">Pending</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>8</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released in-progress">Work In Progress</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                      <tr>
                        <td>9</td>
                        <td>Adminty Admin v1</td>
                        <td>14/05/2022</td>
                        <td>01/01/2015</td>
                        <td><span class="released comming">Coming soon</span></td>
                        <td>5 Hours 20 Minutes</td>
                        <td>Coderthemes</td>
                        <td>
                          <span class="delete view"><i class="fas fa-binoculars" aria-hidden="true"></i></span>
                          <span class="delete edit"><i class="fas fa-pencil-alt" aria-hidden="true"></i></span>
                          <span class="delete delete-btn"><i class="far fa-trash-alt" aria-hidden="true"></i></span>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>


@endsection 